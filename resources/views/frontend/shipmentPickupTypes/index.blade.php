        ...
        <div class="col-md-12">
            @can('shipment_pickup_type_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.shipment-pickup-types.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $shipmentPickupType->id }}">
                                        ...
                                        <td>
                                            @can('shipment_pickup_type_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.shipment-pickup-types.show', $shipmentPickupType->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('shipment_pickup_type_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.shipment-pickup-types.edit', $shipmentPickupType->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('shipment_pickup_type_delete')
                                                <form action="{{ route('frontend.shipment-pickup-types.destroy', $shipmentPickupType->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('shipment_pickup_type_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.shipment-pickup-types.massDestroy') }}",
