        ...
        <div class="col-md-12">
            @can('importer_info_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.importer-infos.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $importerInfo->id }}">
                                        ...
                                        <td>
                                            @can('importer_info_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.importer-infos.show', $importerInfo->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('importer_info_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.importer-infos.edit', $importerInfo->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('importer_info_delete')
                                                <form action="{{ route('frontend.importer-infos.destroy', $importerInfo->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('importer_info_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.importer-infos.massDestroy') }}",
