        ...
        <div class="col-md-12">
            @can('shipment_tracking_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.shipment-trackings.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $shipmentTracking->id }}">
                                        ...
                                        <td>
                                            @can('shipment_tracking_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.shipment-trackings.show', $shipmentTracking->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('shipment_tracking_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.shipment-trackings.edit', $shipmentTracking->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('shipment_tracking_delete')
                                                <form action="{{ route('frontend.shipment-trackings.destroy', $shipmentTracking->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('shipment_tracking_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.shipment-trackings.massDestroy') }}",
